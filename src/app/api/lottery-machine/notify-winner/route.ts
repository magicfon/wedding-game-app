...
export async function POST(request: NextRequest) {
    try {
        ...
        // 檢查是否啟用中獎通知（使用 lottery_state 表作為唯一設定來源）
        ...
        // 只有明確設定為 true 時才發送通知
        const notifyEnabled = lotteryState?.notify_winner_enabled === true
        ...
        if (process.env.LINE_CHANNEL_ACCESS_TOKEN && process.env.LINE_CHANNEL_SECRET) {
            ...
            if (lotteryRecord.winner_photo_url) {
                try {
                    ...
                    await client.pushMessage(winnerLineId, {
                        ...
                        contents: {
                            ...
                            hero: {
                                type: 'image',
                                url: lotteryRecord.winner_photo_url,
                                size: 'full',
                                aspectRatio: '20:13',
                                aspectMode: 'cover',
                                action: {
                                    type: 'uri',
                                    label: '查看照片',
                                    uri: lotteryRecord.winner_photo_url
                                }
                            },
                            body: {
                                type: 'box',
                    ...
                    await client.pushMessage(winnerLineId, {
                        type: 'text',
                        text: `🎉 恭喜您中獎！\n\n您在照片抽獎活動中被選中！\n\n中獎時間：${timeString}\n\n照片連結：${lotteryRecord.winner_photo_url}`
                    })
                }
            } else {
                // 降級發送純文字
