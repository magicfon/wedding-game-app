...
async function fetchAllData() {
  console.log('[export-offline] Fetching data from Supabase...')
  ...
  if (usersErr) console.error('[export-offline] users error:', usersErr.message)
  console.log(`[export-offline] Users: ${users?.length || 0}`)

  // 2. 照片 + 上傳者
  const { data: rawPhotos, error: photosErr } = await supabaseAdmin
    .from('photos')
    .select(`
  ...
  if (photosErr) console.error('[export-offline] photos error:', photosErr.message)
  console.log(`[export-offline] Photos: ${rawPhotos?.length || 0}`)
  ...
  if (questionsErr) console.error('[export-offline] questions error:', questionsErr.message)
  console.log(`[export-offline] Questions: ${questions?.length || 0}`)

  // 4. 答題記錄
  const { data: answerRecords, error: answersErr } = await supabaseAdmin
    .from('answer_records')
    .select(`
      *,
    ...
    `)
    .order('created_at', { ascending: false })
  if (answersErr) console.error('[export-offline] answer_records error:', answersErr.message)
  console.log(`[export-offline] Answer records: ${answerRecords?.length || 0}`)
  ...
  // 6. 抽獎歷史
  const { data: lotteryHistory, error: lotteryErr } = await supabaseAdmin
    .from('lottery_history')
    .select('*')
    .order('draw_time', { ascending: false })
  if (lotteryErr) console.error('[export-offline] lottery error:', lotteryErr.message)
  console.log(`[export-offline] Lottery history: ${lotteryHistory?.length || 0}`)

  // 7. 照片投票紀錄（含投票者資訊）
  const { data: photoVotes, error: photoVotesErr } = await supabaseAdmin
    .from('votes')
    .select(`
      id, photo_id, voter_line_id, created_at,
      voter:users!votes_voter_line_id_fkey(display_name, avatar_url)
    `)
    .order('created_at', { ascending: false })
  if (photoVotesErr) console.error('[export-offline] votes error:', photoVotesErr.message)
  console.log(`[export-offline] Photo votes: ${photoVotes?.length || 0}`)

  // 8. 婚紗照投票紀錄（含投票者資訊）
  let weddingVotes: any[] = []
  // 先嘗試帶 FK join，若失敗則退回只抓 votes 再手動配對用戶
  const { data: wvData, error: wvErr } = await supabaseAdmin
    .from('wedding_photo_votes')
    .select(`id, photo_id, voter_line_id, created_at`)
    .order('created_at', { ascending: false })
  if (wvErr) {
    console.error('[export-offline] wedding votes error:', wvErr.message)
  } else {
    // 手動配對用戶名稱
    ...
    weddingVotes = (wvData || []).map((v: any) => ({
      ...
      voter: userMap.get(v.voter_line_id) ? {
        display_name: userMap.get(v.voter_line_id).display_name,
        avatar_url: userMap.get(v.voter_line_id).avatar_url,
      } : null
    }))
  }
  console.log(`[export-offline] Wedding photo votes: ${weddingVotes.length}`)
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`[export-offline] Wedding photos: ${weddingPhotos.length}`)
    }
  } catch (e) {
    console.error('[export-offline] Google Drive error:', e)
  }
  ...
  return {
    ...
    weddingVotes: (weddingVotes || []).map((v: any) => ({
      ...v,
      voter: Array.isArray(v.voter) ? v.voter[0] : v.voter
    })),
    weddingPhotos
...
function generateRankingsPage(data: any): string {
  ...
  const rows = sorted.map((user: any, i: number) => {
    ...
    return `<div class="card" style="padding:14px 20px;margin-bottom:10px;display:flex;align-items:center;gap:14px;${bg}">
      ...
      <div style="flex:1;">
        <div style="font-weight:600;">${escapeHtml(user.display_name || '匿名')}</div>
      </div>
...
function generateVoteRecordsPage(data: any): string {
  ...
  // 按投票數排序照片
  const sortedPhotoIds = Object.keys(votesByPhoto)
    .map(Number)
  ...
  const weddingVoteCards = sortedWeddingIds.map(photoId => {
    ...
    return `<div class="card" style="margin-bottom:20px;">
          <div style="display:flex;align-items:center;gap:14px;padding:16px;border-bottom:2px solid #f3f4f6;">
            ...
            <div style="flex:1;">
              <div style="font-weight:700;font-size:1.05rem;">${wpName}</div>
            </div>
  ...
  const userSummaryRows = sortedUsers.map(u => {
    return `<div class="card" style="padding:12px 20px;margin-bottom:8px;display:flex;align-items:center;gap:12px;">
